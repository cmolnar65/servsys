...
public class SyncCallNotes
{
	...
	private String paytype;
        ...
        public SyncCallNotes (Connection con, Connection conu)
		throws SQLException, TodoException
	{
		Statement stmt = con.createStatement();
		Statement stmt2 = con.createStatement();
		Statement stmtu = conu.createStatement();
	ResultSet rs = stmt.executeQuery("select * from callnotes where (servsync=0 or servsync is NULL) and ndate is not NULL;");
	...
	//	stmtu.executeUpdate("INSERT INTO time_sheet (tdate, callslip, customer, time_in, time_out, item_sold, amount, amount_collected, commision, dispatch_time, citem_sold, camount, ctype, login, paytype) Values ('" + tdate + "','" +callslip+ "','"+ tcustname + "','"+ time_in + "','" + time_out + "','"+ isold + "','"+ amount +"','"+amount_collected+"','"+ commision+"','"+ dispatch_time + "', '"+ cisold + "','"+ camount +"','"+ctype+"', '"+login+"', '"+paytype+"')");
		stmtu.executeUpdate("INSERT INTO callnotes (custnum, sitenum, userlogin, ndate, callslip, note, servsync) Values ('" + custnum + "','" + sitenum + "','"+userlogin+"','"+ndate+"', '"+callslip+"', '"+note+"', '"+servsync+"')");
		stmt2.executeUpdate("update callnotes set servsync=2 where recnum="+recnum+";");
		}
	}
